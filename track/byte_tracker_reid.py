...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, feat, feat_buffer_size=100):
        super().__init__()
        # wait activate
...
class BYTETracker_reid(object):
    ...
    def update(self, output_results, img_info, img_size, output_features):
        self.frame_id += 1
        activated_starcks = []
        refind_stracks = []
        lost_stracks = []
        ...
        feature_keep = output_features[remain_inds]
        feature_second = output_features[inds_second]

        if len(dets) > 0:
            '''Detections'''
            detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, f) for
                          (tlbr, s, f) in zip(dets, scores_keep, feature_keep)]
        else:
            detections = []
        ...
        if len(dets_second) > 0:
            ...
            detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, f) for
                          (tlbr, s, f) in zip(dets_second, scores_second, feature_second)]
        else:
            detections_second = []
